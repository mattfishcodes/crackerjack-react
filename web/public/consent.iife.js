...
;(() => {
  ...
  function r(e) {
    let n = document.cookie
      ...
      ?.split('=')[1]
    return n === void 0 ? null : JSON.parse(n)
  }
  function h(e, t, n) {
    document.cookie = `${t}=${JSON.stringify(e)}; SameSite=None; Secure;${n ? ` domain=${n};` : ''} path=/`
  }
  function c(e, t, n) {
    ;(h(e, t, n),
      s(e),
      window.dispatchEvent(new CustomEvent('consent:change', { detail: e })))
  }
  var o = 'site_consent_v1'
  function d({ gtmId: e }) {
    ;((window.dataLayer = window.dataLayer || []),
      window.dataLayer.push({ event: 'gtm.js', 'gtm.start': Date.now() }))
    let t = document.createElement('script')
    ;((t.async = !0),
      (t.id = 'google-tag-manager'),
...
`
  ...
  `
    let n = t.querySelector('[data-close]')
    n &&
      (n.onclick = () => {
        t.querySelector('#banner')?.classList.remove('visible')
      })
    let a = t.querySelector('[data-accept]')
    a &&
      (a.onclick = () => {
        ;(window.cookieConsent.acceptAnalytics(),
          t.querySelector('#banner')?.classList.remove('visible'))
      })
    let m = t.querySelector('[data-reject]')
    ;(m &&
      (m.onclick = () => {
      ...
      setTimeout(() => {
        t.querySelector('#banner')?.classList.add('visible')
      }, 300))
  }
  function l(e) {
    window.cookieConsent = {
      ...
      acceptAnalytics() {
        c({ analytics: !0, marketing: !1 }, o, e)
      },
      rejectAll() {
        c({ analytics: !1, marketing: !1 }, o, e)
      },
      open() {
        i()
      },
  ...
  window.onload = () => {
    g({ gtmId: f?.dataset.gtm, domain: f?.dataset.domain })
  }
})()
